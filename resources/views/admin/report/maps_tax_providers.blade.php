    ...
    <div class="xs-pd-20-10 pd-ltr-20">
        ...
        <div class="pd-20 card-box mb-30">
            <div class="clearfix">
                ...
                <form action="{{url("reports/core/maps_tax_providers")}}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="row">
                ...
                <tbody>
                    @foreach ($collection as $item)
                        @if (empty($item->document_id))
                            <tr>
                                <td class="table-plus">{{$item->invoices->type." ".$item->invoices->number}}</td>
                                <td>{{(empty($item->invoices->providers->name))?"Consumidor Final" : $item->invoices->providers->name}}</td>
                                <td>{{$item->percent}}%</td>
                                <td>{{number_format($item->total,2,".",",")}}</td>
                                <td>{{$item->invoices->users->name}}</td>
                                <td>{{$item->date}}</td>
                            </tr>
                        @endif
                    @endforeach
